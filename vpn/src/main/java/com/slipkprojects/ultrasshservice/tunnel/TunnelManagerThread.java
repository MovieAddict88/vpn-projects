	...
	*/
	...
	protected void startClienteSSH() throws Exception {
		...
		try {
            ...
            SkStatus.logInfo("<strong><html><font color='#008A00'>" + mContext.getString(R.string.state_connected) + "</font></html></strong>");
...
@Override
	...
	public void onReceiveInfo(int id, String msg) {
		if (id == SERVER_BANNER) {
			if (!mConfig.getPrivString("SIRBIRNETH").equals("")){
				SkStatus.logInfo("<strong>" + mContext.getString(R.string.log_server_banner) + "</strong> " + mConfig.getPrivString("SIRBIRNETH"));
			} else {
