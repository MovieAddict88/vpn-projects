					...
					for (String app_pacote : filterApps) {
						try {
							if (filterBypassMode) {
								...
								mHostService.onDiagnosticMessage(String.format("Apps Filter: Vpn disabled for \"%s\"", app_pacote));
							}
							else {
								builder.addAllowedApplication(app_pacote);
								mHostService.onDiagnosticMessage(String.format("Apps Filter: Vpn enabled for \"%s\"", app_pacote));
							}
						} catch(PackageManager.NameNotFoundException e) {
							mHostService.onDiagnosticMessage("App \"" + app_pacote + "\" not found. Apps filter will not work, check the settings.");
						}
					}
				}
			}
