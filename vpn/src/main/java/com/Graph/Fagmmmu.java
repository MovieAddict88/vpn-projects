...
public final class Fagmmmu extends View {
    ...
    public Fagmmmu(Context context) {
        super(context);
        init(null, 0);
    }

    public Fagmmmu(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public Fagmmmu(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.updateTimer.scheduleAtFixedRate(new TimerTask() {
				public void run() {
					synchronized (Fagmmmu.this) {
						...
						if (Fagmmmu.this.last_download < 0) {
							Fagmmmu.this.last_download = current_download;
						}
						if (Fagmmmu.this.last_upload < 0) {
							Fagmmmu.this.last_upload = current_upload;
						}
						Fagmmmu.this.download = current_download - Fagmmmu.this.last_download;
						Fagmmmu.this.upload = current_upload - Fagmmmu.this.last_upload;
						if (Fagmmmu.this.download < 0) {
							Fagmmmu.this.download = 0;
						}
						if (Fagmmmu.this.upload < 0) {
							Fagmmmu.this.upload = 0;
						}
						Fagmmmu.this.samples.add(0, new ShadowBandwidth(Fagmmmu.this.download, Fagmmmu.this.upload, Fagmmmu.this.internet_type));
						while (Fagmmmu.this.samples.size() > 100) {
							Fagmmmu.this.samples.remove(Fagmmmu.this.samples.size() - 1);
						}
						Fagmmmu.this.last_download = current_download;
						Fagmmmu.this.last_upload = current_upload;
					}
					((Activity) Fagmmmu.this.getContext()).runOnUiThread(new Runnable() {
							public void run() {
								Fagmmmu.this.invalidate();
							}
						});
				}
			}, 1000, 1000);
